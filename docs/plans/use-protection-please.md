# rusty_rtos_backoff — hardening audit
...
**Unit**: `rusty_rtos_backoff` — package (facade + `no_std` core)
**Tier**: standard — a pure computation (the exponential-backoff-with-jitter algorithm over caller-supplied integers); no bytes from a wire, a store or a bus enter it
...
| H-02 | Threat model revisited after last major change | N/A | standard tier: a critical-only gate (registry tier column); not in the denominator | |
...
| H-20 | ★ Secrets zeroized; never logged | N/A | standard tier: a critical-only gate (registry tier column); not in the denominator | |
| H-21 | Concurrency discipline | N/A | standard tier: a critical-only gate (registry tier column); not in the denominator | |

### Phase 4 — Static analysis

| ID | Gate | Status | Evidence | Target |
|---|---|---|---|---|
| H-22 | Static analysis beyond the default linter runs on every PR | N/A | standard tier: a critical-only gate (registry tier column); not in the denominator | |
...
| ID | Gate | Status | Evidence | Target |
|---|---|---|---|---|
| H-30 | Proof of panic-freedom / UB-freedom per `unsafe` module | N/A | standard tier: a critical-only gate (registry tier column); not in the denominator | |
